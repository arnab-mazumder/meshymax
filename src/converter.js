...
/**
 * Converts a .meshy or .glb binary buffer into a standalone, uncompressed .glb buffer.
 * 
 * @param {ArrayBuffer | Uint8Array | Buffer} inputData - Input binary data of .meshy or .glb file.
 * @returns {Promise<Buffer>} - Standard GLB binary buffer ready to be saved/downloaded.
 */
export async function convertMeshy(inputData) {
  ...
  // Normalize input to ArrayBuffer / Uint8Array
  let arrayBuffer;
  if (inputData instanceof ArrayBuffer) {
    ...
    throw new Error('Unsupported data format. Expected ArrayBuffer or TypedArray/Buffer.');
  }
  ...
  // Return Node Buffer / Uint8Array wrapper
  return Buffer.from(finalGlbBuffer);
}
